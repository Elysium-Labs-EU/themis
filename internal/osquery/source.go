...
// lynis and themis-native, matching the historical order. cfg.OsqueryEnabled
// gates whether it runs at all — an operator config file's
// sources.osquery.enabled: false leaves it out of the enabled set
// entirely; when enabled it still no-ops on a host with no prior apply
// state or no osqueryi.
func init() {
	audit.Register("osquery", 30, func(cfg audit.SourceConfig) (audit.Source, error) {
		if !cfg.OsqueryEnabled {
